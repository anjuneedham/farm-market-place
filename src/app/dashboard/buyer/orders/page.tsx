...
export default async function BuyerOrdersPage() {
  const { user } = await requireRole('BUYER', '/dashboard/buyer/orders');
  const orders = db.orders.forUser(user.id, 'buyer', { perPage: 60 });

  return (
    <DashboardShell title="Orders" nav={buyerNav()} activeHref="/dashboard/buyer/orders">
      {orders.items.length > 0 ? (
        <div className="space-y-3">
          {orders.items.map((order) => {
            const alreadyReviewed = db.reviews.byOrderAndAuthor(order.id, user.id);
            return (
              <div key={order.id} className="rounded-lg border border-line bg-surface p-4">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div>
                    <p className="font-medium text-ink-900">
                      {order.reference} · {order.sellerName}
                    </p>
                ...
                <div className="mt-3 flex items-center justify-between">
                  ...
                  {order.status === 'COMPLETED' && !alreadyReviewed ? (
                    <ReviewPrompt orderId={order.id} sellerName={order.sellerName} />
                  ) : null}
                </div>
              </div>
            );
        ...
        <EmptyState
          title="No orders yet."
          description="Orders sellers record with you will appear here."
